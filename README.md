...
Each competition problem gives you a few input/output examples and a query. The model has to figure out the hidden rule and produce the correct output inside `\boxed{}`.

The obvious approach is to prompt a strong LLM to generate reasoning traces and filter by correctness. The problem is that even when the final answer is right, the intermediate steps are inconsistent across problems, written differently each time, and sometimes subtly wrong. Training on that kind of data teaches the model to sound confident rather than to follow a procedure.

The approach here is different. For each problem category, a deterministic Python solver mirrors the exact reasoning process step by step and emits natural-language text as it runs. The solver is the trace generator. Every training example is verified (`extract_answer(trace) == ground_truth`) before inclusion, so no noisy or hallucinated steps ever enter training.
...
| `numeral` | Arabic to Roman | Greedy subtraction walk through standard Roman value table |
| `cipher` | Substitution cipher | Build bijective letter-to-letter map word-by-word using an Alice in Wonderland word list |
| `bit_manipulation` | Per-bit 8-bit operation | Column-matching across Identity / NOT / AND / OR / XOR / shifts with stride extrapolation |
| `equation_numeric` | Arithmetic on parsed expressions | Try common and rare operations (div, mod, digit-level ops) with reversal variants |
| `cryptarithm_deduce` | Symbol-to-digit mapping + math | 7-family DSL cascade: concat, set ops, position ops, unary ops, char map, bit ops, arithmetic |
...
uv run python3 train_sft.py      # 4. SFT training on Tinker
uv run modal run upload_adapter.py   # 5. Push LoRA adapter to Kaggle
```

To train on Kaggle T4 GPUs instead of steps 4 and 5:
...
| Learning rate | 2×10⁻⁴ to 0 (step-linear decay) |
...
Computes the constant (k or conversion factor) for each example pair, sorts the values, picks the median, and multiplies through for the query. Uses long-division and long-multiplication helpers in `store_types.py` to show digit-level arithmetic in the trace.

### `cipher`
Builds a bijective substitution mapping word-by-word. For each cipher word, it generates candidates from a word list (derived from Alice in Wonderland) whose letter pattern matches and that are consistent with the partial mapping built so far. The trace shows each word, the known mappings at that point, and the candidate chosen.
...
5. **Char map** - learn a bijective symbol-to-symbol substitution from examples (28 key patterns)
...
7. **Arithmetic** - symbol-to-digit mapping + math operation (add, sub, mul, div, mod, abs_diff, gcd, max, min) with reversal variants
...
Also handles problems where negative results are encoded with the operator character as a prefix or suffix (e.g. `5-` instead of `-5`), strips the encoding for matching, and re-applies it in the trace.


## What Didn't Work

**LLM distillation**: trying to get Claude to write the reasoning traces produced correct final answers but inconsistent styles. Sometimes it would work backward, sometimes skip steps, sometimes add wrong intermediate reasoning even when the boxed answer was right. Training on that mix meant the model never committed to a single strategy.

**Z3 SMT solver**: Z3 finds the digit assignment but not the reasoning trace. Reconstructing a natural-language chain-of-thought from Z3's internal proof state is not straightforward, and the traces it produced didn't match how the model should reason at test time.

**AC-3 constraint propagation**: AC-3 is designed for problems with rich initial constraints. In the competition format (4 symbols, 5-char input, 2-5 examples), the initial domains are nearly unconstrained and it degenerates to backtracking anyway, without solving the trace generation problem.

The thing all three approaches have in common is that the trace and the solver are separate. You compute the answer one way and describe the reasoning another. That gap is the problem. The DSL cascade here collapses it: the solver runs, emits text at each step, and the trace is just what the solver printed.
...
```
reasoners/              # Per-category solvers (emit the training trace as they run)
...
train_sft.py            # Stage 4: SFT training loop on Tinker
...
trainer/client.py       # ServiceClient wrapping Tinker and Modal backends

upload_adapter.py           # Push LoRA adapter via Modal to Kaggle model registry
...
cp .env.example .env
# fill in TINKER_API_KEY and KAGGLE_API_KEY
```

The tokenizer is downloaded automatically from HuggingFace on the first run of `corpus.py`.
